...
export default function PbnLinks() {
    ...
    const getPbnLinks = async () => {
        const { data } = await axios.get('/api/pbn-links/get-all', { params: linksQuery });
    ...
    const addPbnWebsite = async () => {
        ...
        setIsLoading(true)
        const { data } = await axios.post('/api/pbn-links/add-new-website', { website: newWebsite });
        setIsLoading(false)
        setNewWebsite('')
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            ...
            {totalPbnLinks > DEFAULT_LIMIT &&
                <Pagination total={totalPbnLinks} limit={linksQuery.limit} setQuery={setQuery}/>
            }
        </div>
    );
}
